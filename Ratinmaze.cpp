...
void helper(vector<vector<int>> &mat, int r, int c, string path, vector<string> &ans, vector<vector<bool>>&vis)
{
    ...
    helper(mat, r - 1, c, path + 'U', ans,vis); // upward
    helper(mat, r + 1, c, path + 'D', ans,vis); // downward
    helper(mat, r , c-1, path + 'L', ans,vis); // leftward
    helper(mat, r, c+1, path + 'R', ans,vis); // rightward
